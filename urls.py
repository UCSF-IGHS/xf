...
urlpatterns = [
    ...
    url(r'^dashboards/(?P<page_id>[-\w]+)/(?P<section>[-\w]+)/(?P<slug>[-\w]+)/$', DashboardPageView.as_view(), name='dashboards'),
    url(r'^%s/(?P<section>[-\w]+)/(?P<slug>[-\w]+)/$' %  (getattr(settings, "DASHGENT_PAGES", 'pages')), DashboardPageView.as_view(), name='pages'),
